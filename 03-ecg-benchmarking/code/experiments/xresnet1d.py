"""
XResNet1d50 Trained from Scratch for ECG Classification
Architecture: XResNet1d50 (expansion=4, layers=[3,4,6,3], kernel_size=5)
              Same architecture as Fraunhofer benchmark, 7-class head (vs pretrained 71-class)
...
Training time estimate:
  GPU : estimation ~30-45 minutes total -- real ~1 hour 30 minutes.
...
# =============================================================================
CONFIG = {
    "experiment_name": "xresnet1d50_ecg_ptbxl_train",
    "model": "XResNet1d50",
    "sampling_rate": 500,
    "signal_length": 5000,
    "num_leads": 12,
    ...
    "kernel_size": 5,       # conv kernel size inside ResBlocks
    "dropout": 0.5,
    "skip_training": False,
}
...
_OUT_DIR = str(_SCRIPT_DIR / "results_xresnet1d_train")
...
def load_ptbxl(data_path=_PTBXL_PATH):
    ...
    print("  PTB-XL: no cache found — loading 500 Hz records (~20-40 min first run)...")
...
# =============================================================================
# XResNet1d50 Architecture (self-contained, no fastai dependency)
# Matches Fraunhofer benchmark architecture: expansion=4, layers=[3,4,6,3]
# =============================================================================
...
class _BottleneckBlock(nn.Module):
    """Bottleneck residual block (expansion=4).
    ni, nf are pre-expansion channel counts; actual channels = ni*expansion, nf*expansion.
    """
    def __init__(self, expansion, ni, nf, stride=1, kernel_size=5):
        super().__init__()
        ni_exp = ni * expansion
        nf_exp = nf * expansion

        self.convs = nn.Sequential(
            _ConvBnRelu(ni_exp, nf,     ks=1,           act=True),
            _ConvBnRelu(nf,     nf,     ks=kernel_size, act=True,  stride=stride),
            _ConvBnRelu(nf,     nf_exp, ks=1,           act=False),
        )

        skip = []
        if stride != 1:
            skip.append(nn.AvgPool1d(2, stride=stride, ceil_mode=True))
        if ni_exp != nf_exp:
            skip.append(_ConvBnRelu(ni_exp, nf_exp, ks=1, act=False))
        self.skip = nn.Sequential(*skip)

        self.act = nn.ReLU(inplace=True)

    def forward(self, x):
        return self.act(self.convs(x) + self.skip(x))


class XResNet1d50(nn.Module):
    """XResNet1d50 trained from scratch for multilabel ECG classification.

    Architecture identical to Fraunhofer benchmark (expansion=4, [3,4,6,3])
    but with a 7-class head instead of 71-class.
    """
    def __init__(self, num_classes=7, input_channels=12, kernel_size=5, dropout=0.5):
        super().__init__()
        expansion = 4
        block_szs = [16, 64, 64, 64, 64]   # pre-expansion; actual = *4 = [64,256,256,256,256]

        self.stem = nn.Sequential(
            _ConvBnRelu(input_channels, 32, ks=5, stride=2),
            _ConvBnRelu(32, 32, ks=5, stride=1),
            _ConvBnRelu(32, 64, ks=5, stride=1),
            nn.MaxPool1d(kernel_size=3, stride=2, padding=1),
        )

        self.layer1 = self._make_group(expansion, block_szs[0], block_szs[1],
                                       n=3, stride=1, ks=kernel_size)
        self.layer2 = self._make_group(expansion, block_szs[1], block_szs[2],
                                       n=4, stride=2, ks=kernel_size)
        self.layer3 = self._make_group(expansion, block_szs[2], block_szs[3],
                                       n=6, stride=2, ks=kernel_size)
        self.layer4 = self._make_group(expansion, block_szs[3], block_szs[4],
                                       n=3, stride=2, ks=kernel_size)

        nf_last = block_szs[-1] * expansion   # 256
        self.pool_bn = nn.BatchNorm1d(nf_last * 2)  # 512 after avg+max concat
        self.head = nn.Sequential(
            nn.Dropout(p=dropout),
            nn.Linear(nf_last * 2, num_classes),
        )
        self._init_weights()

    def _make_group(self, expansion, ni, nf, n, stride, ks):
        return nn.Sequential(*[
            _BottleneckBlock(expansion, ni if i == 0 else nf, nf,
                             stride=stride if i == 0 else 1, kernel_size=ks)
            for i in range(n)
        ])
    ...
    def forward(self, x):
        x = self.stem(x)      # (B, 64, 1250) for 500 Hz input
        x = self.layer1(x)    # (B, 256, 1250)
        x = self.layer2(x)    # (B, 256, 625)
        x = self.layer3(x)    # (B, 256, 313)
        x = self.layer4(x)    # (B, 256, 157)

        avg = F.adaptive_avg_pool1d(x, 1)
        mx  = F.adaptive_max_pool1d(x, 1)
        x = torch.cat([avg, mx], dim=1).squeeze(-1)   # (B, 512)
        x = self.pool_bn(x)
...
if __name__ == "__main__":
    ...
    summary_path = os.path.join(_OUT_DIR, "results_summary_xresnet1d_train.json")
